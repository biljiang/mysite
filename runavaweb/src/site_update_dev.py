...
for start_index in range((len(text)-maxlen-num_of_days),(len(text)-maxlen)):
    for diversity in [1.0]:
        ...
        print ('---------------------generating with seed------------------------',sentence.index[-1])
        ###print (generated)
...
#line_initial['Index_predicted']= np.multiply(line_initial.Close.shift(1),(1+line_initial.top_pred))
#duplicate and save line_initial table as is
predicted_table=line_initial.copy()

tracking_file = open('../data/tracking_record.pkl', 'rb')
...
output = open('../data/tracking_record.pkl', 'wb')
pickle.dump(tracking_record, output)
output.close()
tracking_record.to_csv('../data/tracking_record.csv')
...
Forecast_table = DataFrame(index=ftable_index,
                           data={'预测指数':["%.2f" %x for x in Index_forecast[1:]],
                                 '可能涨跌幅#1':["%.2f%%" %(x*100) for x in top1_preds],
                                 '涨跌幅#1概率':["%.2f%%" %(x*100) for x in top1_probs],
                                 '可能涨跌幅#2':["%.2f%%" %(x*100) for x in top2_preds],
                                 '涨跌幅#2概率':["%.2f%%" %(x*100) for x in top2_probs]})
Forecast_table=Forecast_table.reindex(
        columns=['预测指数','可能涨跌幅#1','涨跌幅#1概率','可能涨跌幅#2','涨跌幅#2概率'])

table_css = open('../tmp/table_css.css', 'r').read()
h=table_css + Forecast_table.to_html(classes='table',index=True)
open('../www/my_table.html', 'w').write(h)
...
open('../www/index_graph1.html','w').write(plot_str)
