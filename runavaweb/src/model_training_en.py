...
# maxium 10 years data from now
#df1 = web.DataReader("SP500", "fred",datetime(1977,1,1),datetime(1986,12,31))
#df=pd.read_csv('../data/GSPC.csv',index_col=0,parse_dates =True)
df=pd.read_csv('../en/data/GSPC.csv',index_col=0,parse_dates =True,na_values=['null'])
...
df_clean= df[(np.abs(df.Return) <=0.08)]
r_clean = df_clean.Return

r_cat, r_bins = pd.cut(r_clean,bins=100,retbins=True)
...
open('../en/tmp/my_model_architecture_en.json', 'w').write(json_string)
model.save_weights('../en/tmp/my_model_weights_en.h5')
'''
# load model
...
import pickle
output = open('../en/tmp/df_parameters1_en.pkl', 'wb')
pickle.dump(chars, output)
pickle.dump(r_bins, output)
pickle.dump(df_grouped_mean, output)
pickle.dump(char_indices, output)
pickle.dump(indices_char, output)
output.close()

'''
output=open('../tmp/df_grouped_mean.json','w')
json_str=df_grouped_mean.to_json()
json.dump(json_str,output)
output.close()

output=open('../tmp/char_indices.json','w')
json_str=char_indices.to_json()
json.dump(json_str,output)
output.close()

###load data

json_str= open('../tmp/df_grouped_mean.json','r').read()
...
plotly.offline.plot(fig, filename='../en/www/index_graph2_en.html')
